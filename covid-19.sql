--- 1. Exploring the covid 19 data set for March
SELECT name, DAY(whn),
...
SELECT name, DATE_FORMAT(whn,'%Y-%m-%d'), (confirmed - LAG(confirmed) OVER (PARTITION BY name ORDER BY whn)) AS 'new this week'
 FROM covid
WHERE name = 'Italy'
AND WEEKDAY(whn) = 0
ORDER BY whn
...
SELECT tw.name, DATE_FORMAT(tw.whn,'%Y-%m-%d'), 
 (tw.confirmed - LAG(tw.confirmed) OVER(PARTITION BY tw.name ORDER BY tw.whn))
...
--- 6. United States has the highest number, Spain is number 2... Notice that while Spain has the second highest confirmed cases, Italy has the second highest number of deaths due to the virus. Include the ranking for the number of deaths in the table.
SELECT 
   name,
   confirmed,
...
SELECT 
   world.name,
   ROUND(100000*confirmed/population,0), RANK() OVER (ORDER BY confirmed/population) AS Rank
